...
public:
    ...
    Thread( bool isStartup = false, ThreadAttr const & attr = DefaultThreadAttr );

    /** \brief 构造函数2
     *
     * 立即创建一个线程，以startRoutine作为线程处理函数、attr为线程属性 */
    Thread( PFN_ThreadFunc startRoutine, void * param, ThreadAttr const & attr = DefaultThreadAttr );
